 ...
 */
export const findSOCKS = async (preferNonHeadless, country, region) => {
    ...
    await Promise.all([
        ...
        page.waitForResponse((response) => response.url().includes('cgi-bin/online.cgi?action=getcountrys') && response.status() === 200)
    ]);
    ...
    const countryLink = await page.evaluate((countryCode) => {
        const countryTd = [
            ...document.querySelectorAll('#getit > table:nth-child(1) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody:nth-child(1) > tr > td:nth-child(2)')
    ...
    await Promise.all([
        ...
        page.waitForResponse((response) => response.url().includes('cgi-bin/online.cgi?country=') && response.status() === 200)
    ]);
    if (region !== undefined) {
    ...
    const headerMessage = await page.$eval('form > table > tbody > tr > td > table > tbody > tr:nth-child(1) > td', (el) => el.textContent);
    ...
    const evalString = await page.evaluate(() => {
        ...
        return decodeURI(firstAvail.href);
    });
    ...
    const proxy = await popup.evaluate(() => document.querySelector('body > form > center > font > input[type=text]').value);
